...
export function IsArrayOfIntHuoString(validationOptions?: ValidationOptions): PropertyDecorator {
  return (target: object, propKey: string | symbol) => {
    ...
    applyDecorators(
      Transform(({ value: val }) => {
        ...
        return val.filter((value) => value !== null && value !== void 0 && value !== '') // 直接去掉, 空''也不能通过正则校验
          .map((value) => {
      ...
      IsString({ message: `${field} 数组项必须是字符串类型`, ...validationOptions }),
      Matches(/^\d+$/, { message: `${field} 数组项必须是整数或整数字符串, 需要>=0`, ...validationOptions })
    )(target, propKey)
  }
}
