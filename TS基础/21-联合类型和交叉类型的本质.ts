...
/* 对象类型的联合类型和交叉类型, 快速记忆, 非对象类型也可以用
(每个对象类型看作一个整体, 不要单独看属性成员)
联合类型: 可以是  既可以是, 又可以是
交叉类型, 必须是  既必须是, 又必须是
保证类型安全 */
...
type 联合 = 坤粉 | 马粉
// 联合是以坤粉{}和马粉{}为并集, 举起手就算, 不管左手右手还是全举手
const a: 联合 = {
  鬼畜: '',
...
type 联合Show = ShowMe<keyof 联合> // 只能是都会的这一个
...
type 交叉Show = ShowMe<keyof 交叉> // 都会
